...
In this project, we investigate the following research question: \textbf{Can language models adapt social acceptability judgments when the same behavior is paired with different cultural norms?}
...
Prior work has shown that language models and benchmarks often encode social assumptions from their data sources. NLPositionality \citep{nlpositionality} argues that datasets reflect the positionality of their creators and annotators. GeoMLAMA \citep{geomlama} shows that commonsense knowledge varies across geographic and cultural contexts. Recent cultural and safety evaluation benchmarks, including SALAD-Bench \citep{saladbench} and NORMAD \citep{normad}, similarly highlight that model behavior may not generalize uniformly across social settings.

Our project also draws on retrieval-augmented generation \citep{rag}, which improves model behavior by providing task-relevant external knowledge at inference time. Instead of retrieving factual passages, we retrieve social norms and rules of thumb. We also use Social-Chem-101 \citep{socialchem101}, a large dataset of social and moral rules of thumb, as a broad norm source for one of our RAG experiments.

\section{Dataset}

We constructed a dataset of 120 manually curated social acceptability examples. Each example contains a scenario or interaction, a culture or social-context label, a cultural-context explanation, and one gold label from \texttt{acceptable}, \texttt{not acceptable}, or \texttt{context-dependent}.
...
\hline
\texttt{acceptable} & 23 \\
\texttt{context-dependent} & 47 \\
\texttt{not acceptable} & 50 \\
\hline
Total & 120 \\
\hline
\end{tabular}
...
Direct three-way classification can make models overcommit to a label too early. We therefore test a 1--10 scalar acceptability prompt. The model first assigns a score, where low scores indicate clear social unacceptability, high scores indicate clear acceptability, and middle scores indicate ambiguity or missing context. We then map scores to labels: 1--3 = \texttt{not acceptable}, 4--7 = \texttt{context-dependent}, and 8--10 = \texttt{acceptable}. This formulation is useful because social acceptability is often gradual rather than categorical.
...
For this experiment, GPT-4o receives the scenario, culture label, cultural-context explanation, and top retrieved Social-Chem-101 norms. The model must output structured JSON including intermediate RAG-grounded findings, whether each norm applies to the scenario, whether the evidence supports a label or only background context, a relevance judgment for the retrieved norms, and the final label. This format helps us audit whether errors are caused by bad retrieval, irrelevant norms, or incorrect model reasoning.
...
\begin{tabular}{llr}
\hline
Experiment & Input / Method & Accuracy \\
\hline
...
Context + RAG v2 + rating & GPT-4o mini, culture-aware RAG + 1--10 rating & 105/120 = 87.50\% \\
Context + RAG v1 + rating & GPT-4o mini, dataset-derived RAG + 1--10 rating & 113/120 = 94.17\% \\
Social-Chem RAG reasoning & GPT-4o, Social-Chem-101 RAG + intermediate findings & 107/120 = 89.17\% \\
Llama 3 8B & [teammate experiment details/results TBD] & TBD \\
Qwen & [teammate experiment details/results TBD] & TBD \\
\hline
\end{tabular}
\caption{Selected experiment results. DeBERTa is excluded from the final comparison because it was not trained or fine-tuned for this task.}
\end{table*}
...
The 1--10 rating prompt improves performance because it gives the model a way to represent borderline cases before mapping to a final label. Many social judgments are not naturally binary. A direct classifier must immediately choose among \texttt{acceptable}, \texttt{not acceptable}, and \texttt{context-dependent}, while the rating setup lets the model first express degree. This is especially helpful for \texttt{context-dependent} cases, where the correct answer often lies between clear acceptability and clear unacceptability.
...
The GPT-4o + Social-Chem-101 RAG reasoning experiment achieved 89.17\% accuracy. Its per-label performance was 82.61\% for \texttt{acceptable}, 85.11\% for \texttt{context-dependent}, and 96.00\% for \texttt{not acceptable}. The model was strongest on clearly unacceptable cases, but it sometimes over-applied broad negative norms to cases labeled \texttt{acceptable} or \texttt{context-dependent}. The structured output helped reveal this pattern: in some privacy cases, the model retrieved valid general norms about consent, but the final prediction became too strict relative to the dataset's cultural-context explanation.

\subsection{Error Categories}

We observed three recurring error types. First, broad rules such as ``breaking agreements is bad'' or ``posting without consent is wrong'' are usually valid, but may be too strong for cases where the cultural context makes the behavior acceptable or conditional. Second, some prompts overuse or underuse \texttt{context-dependent}; they either classify ambiguous cases as clearly unacceptable because a retrieved norm sounds negative, or classify clearly unacceptable cases as context-dependent because missing details are imaginable. Third, RAG helps only when the retrieved examples align with the scenario's core norm; if retrieval finds a related but not exact norm, the model may reason from the wrong social variable.
